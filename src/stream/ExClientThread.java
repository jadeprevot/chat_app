...
class ExClientThread implements Runnable
{
	...
	private ExServer _blablaServ; // pour utilisation des méthodes de la classe principale
	...
	ExClientThread(Socket s, ExServer blablaServ) // le param s est donnée dans BlablaServ par ss.accept()
	{
			...
			while(_in.read(charCur, 0, 1)!=-1) // attente en boucle des messages provenant du client (bloquant sur _in.read())
			{
				...
				else if(!message.equalsIgnoreCase("")) // juste une vérification de principe
				{
					if(charCur[0]=='\u0000') // le dernier caractère était '\u0000' (char de terminaison nulle)
						...
						_blablaServ.sendAll("sdsdds",""+charCur[0]);
					else _blablaServ.sendAll("sdsddsdsdsdsd",""); // sinon on envoie le message à tous
